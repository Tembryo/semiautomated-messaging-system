...
with open(file_log, 'wb') as logfile:
  with open(file_in, 'rb') as csvfile:
	...
	connection.execute("INSERT INTO Contacts (FirstName, LastName, Company, CommunicationState, EmailAddress, DateContacted) VALUES (?, ?, ?, 0, '', date('now'))", (row["First Name"], row["Last Name"], row["Company"]))
